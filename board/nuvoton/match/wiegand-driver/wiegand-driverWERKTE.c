...
static ssize_t wiegand_value_store(struct class *cls, struct class_attribute *attr, const char *buf, size_t count);
static ssize_t wiegand_value_show(struct class *cls, struct class_attribute *attr, char *buf);
static ssize_t reader2_show(struct class *cls, struct class_attribute *attr, char *buf);
...
static void refresh_timer_function(unsigned long data)
{
	...
	sysfs_notify(wiegandKObj, NULL, "notify");
	printk("sysfs_notify");
...
{
	unsigned int mask = 0;
	printk(KERN_INFO "wiegand_poll called.\n");
	printk("XXXwiegand_poll called.\n");
	poll_wait (file, &read_waitq, wait ); // Data is not readable and will not sleep immediately
	printk("XXXwiegand_poll wakup.\n");
...
static int device_fasync(int fd, struct file *pfile, int mode)
{
    printk("In FASYNC function\n");
    return fasync_helper(fd, pfile, mode, &my_fasync);
}
...
//unsigned long wiegand_value = 111;
    //last_reader_nr;
static ssize_t wiegand_value_show(struct class *class,
                                struct class_attribute *attr,
                                char *buf) {
        return scnprintf(buf, PAGE_SIZE, "%lu\n", keycode);
}

static ssize_t wiegand_value_store(struct class *class,
                                struct class_attribute *attr,
...
static struct class_attribute wiegand_class_attrs[] = {
  //__ATTR(value, S_IWUSR | S_IRUGO, wiegand_value_show, wiegand_value_store),
  __ATTR(reader1, S_IWUSR | S_IRUGO, wiegand_value_show, wiegand_value_store),
  __ATTR_RO(reader2),
  __ATTR_NULL,
};
...
static int __init wiegand_init(void)
{
	...
	class_create_file(cls,wiegand_class_attrs);
