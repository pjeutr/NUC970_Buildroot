...
function find_ledgers() {
    return find_objects_by_sql("SELECT * FROM ledger ORDER BY id DESC LIMIT 500");
}
...
function update_ledger($user_obj, $readerId, $controller) {
    ...
    if($controller->apb == 1) { //only include in ledger if this is a apb controller
        if($readerId == 1) {
